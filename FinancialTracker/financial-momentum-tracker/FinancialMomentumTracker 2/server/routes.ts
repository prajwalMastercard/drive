...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Route to serve the PowerPoint template file
  app.get('/api/download/ppt/recommendation-insights', (req, res) => {
    try {
      // Construct the path to the PowerPoint file - use no spaces in filename
      const filePath = path.resolve(process.cwd(), 'attached_assets', 'RecommendationsInsights.pptx');
      
      console.log('Serving PowerPoint file from:', filePath);
      
      // Attempt to list the files in the directory for debugging
      try {
        const attachedAssetsDir = path.resolve(process.cwd(), 'attached_assets');
        const files = fs.readdirSync(attachedAssetsDir);
        console.log('Files in attached_assets directory:', files);
      } catch (e) {
        console.error('Error listing attached_assets directory:', e);
      }
      ...
      if (!fs.existsSync(filePath)) {
        ...
        return res.status(404).json({ 
          error: 'PowerPoint template file not found'
        });
      }
      ...
      // Set the appropriate headers for a PowerPoint file
      ...
      res.status(500).json({ 
        error: 'An error occurred while serving the PowerPoint file'
      });
    }
  });
